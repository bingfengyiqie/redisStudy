...
public class Chapter02 {
    ...
    private static final String IP = "172.31.0.51";
    ...
    private void test(Jedis conn) {
        sop("-------------------------------");
        // sop(conn.select(1));// 测试是否连接上
        // sop(conn.hset("login:", "jim", UUID.randomUUID().toString()));
        // for (int i = 0; i < 10; i++) {
        // sop(conn.hset("login:", "a" + i, "b" + i));
        // }
        sop("-------------------------------");
        sop(conn.hget("login:", "jim"));
        for (int i = 0; i < 10; i++) {
            sop(conn.hget("login:", "a" + i));
        }
    }
    ...
    public void testLoginCookies(Jedis conn) throws InterruptedException {
        ...
        updateToken(conn, token, "xiaoming", "itemX");
        System.out.println("We just logged-in/updated token: " + token);
        System.out.println("For user: 'xiaoming'");
        System.out.println();
    ...
    public void updateToken(Jedis conn, String token, String user, String item) {
        ...
        // 使用一个散列存储：令牌与已经登录的用户之间的映射
        conn.hset("login:", token, user);
        // 使用一 个有序集合存储：令牌最后一次出现的时间
        conn.zadd("recent:", timestamp, token);
        // 浏览的商品条目
        if (item != null) {
            conn.zadd("viewed:" + token, timestamp, item);
            // 移除旧的记录，只保持用户最近浏览的25个商品
    ...
    public class CleanSessionsThread extends Thread {
        ...
        public void run() {
            while (!quit) {
                long size = conn.zcard("recent:");// 返回基数 总数量
                if (size <= limit) {
                    try {
                        sleep(1000);
